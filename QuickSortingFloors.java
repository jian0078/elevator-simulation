...
public class QuickSortingFloors{
	 ...
	 */
	public static void sort( int[] array, final int size,MovingState direction){
		dirCall=direction;
 ...
 */
	public static void sortRecursive( int[] array, final int low, final int high){
		...
		int pivot=0;
		if(dirCall==MovingState.Up)
		{
			pivot=partitionUp(array,low,high);
		}
		
		if(dirCall==MovingState.Down)
		{
			pivot=partitionDown(array,low,high);
		}
	 ...
	 * @return
	 */
	private static int partitionUp( int[] array, final int low, final int high){
	...
	private static int partitionDown( int[] array, final int low, final int high){
		int pivot = array[high];// choose last element as pivot
		int i = (low - 1); // assume left most element is smallest
		try {
		for( int j = low; j < high; j++){// start at smallest index
	 ...
	 */
	public static void swap( int[] array, final int source, final int dest){
		try {
